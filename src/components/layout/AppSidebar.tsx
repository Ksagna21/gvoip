  ...
  return (
    <div className="h-14 flex items-center justify-between px-4 bg-transparent dark:bg-transparent z-50">
      ...
      <div className="flex-1 flex items-center justify-end gap-3">
        ...
        <button
          onClick={toggle}
          className={cn("w-9 h-9 flex items-center justify-center", "active:scale-95 transition-all duration-200")}
          title={dark ? "Passer en mode clair" : "Passer en mode sombre"}
        >
          {dark ? <Sun size={18} className="text-cyan-400" /> : <Moon size={18} className="text-muted-foreground" />}
        </button>
      </div>
    </div>
  );
...
const SidebarContent = ({ isCollapsed, onCollapse }: { isCollapsed: boolean; onCollapse: () => void }) => {
  ...
  // Nom affiché : display_name > partie locale de l'email > "Utilisateur"
  ...
  const navItems = [
    ...
    { to: "/countries",   icon: Globe,           label: "Pays",            show: isAdmin || canView("dashboard_country"),            section: "menu" },
    { to: "/ipbx",        icon: Server,          label: "IPBX",            show: isAdmin || canView("settings"),                     section: "menu" },
    { to: "/sip-trunks",  icon: Network,         label: "SIP Trunks",      show: isAdmin || canView("sip_trunks"),                   section: "menu" },
    { to: "/extensions",  icon: Phone,           label: "Extensions",      show: isAdmin || canView("extensions"),                   section: "menu" },
    { to: "/calls",       icon: PhoneCall,       label: "Appels",          show: isAdmin || canView("cdr"),                          section: "menu" },
    { to: "/quality",     icon: Activity,        label: "Qualité VoIP",    show: isAdmin || canView("quality"),                      section: "general" },
    { to: "/alerts",      icon: Bell,            label: "Alertes",         show: isAdmin || canView("alerts"),                       section: "general" },
  ...
  return (
    <>
      ...
      <nav className="flex-1 overflow-y-auto py-6 px-3 space-y-8 scrollbar-thin">
        {menuItems.length > 0 && (
          <div>
            {!isCollapsed && <p className="px-3 mb-3 text-[10px] dark:text-white text-muted-foreground/50">MENU</p>}
        ...
        {generalItems.length > 0 && (
          <div>
            {!isCollapsed && <p className="px-3 mb-3 text-[10px] dark:text-white text-muted-foreground/50">GÉNÉRAL</p>}
            <div className="space-y-1">
              {generalItems.map((item) => <NavItem key={item.to} item={item} />)}
            </div>
          </div>
        )}
      </nav>

      {/* Footer Profil — affiche avatar + display_name en temps réel */}
      <div className="p-4">
        {!isCollapsed ? (
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3 min-w-0">
              <Avatar className="w-9 h-9 rounded-2xl shrink-0">
                <AvatarImage src={profile?.avatar_url || undefined} alt={displayName} />
                <AvatarFallback className="rounded-2xl bg-primary/10 font-bold text-primary text-lg">
                  {initials}
                </AvatarFallback>
              </Avatar>
            ...
            <Button
              ...
              size="icon"
              onClick={signOut}
              className="h-9 w-9 text-muted-foreground hover:text-red-500 hover:bg-red-500/10 rounded-xl shrink-0"
            >
              <LogOut size={18} />
            </Button>
          </div>
        ) : (
          /* Mode replié : avatar seul */
          <div className="flex flex-col items-center gap-2">
            <Avatar className="w-9 h-9 rounded-2xl">
              <AvatarImage src={profile?.avatar_url || undefined} alt={displayName} />
              <AvatarFallback className="rounded-2xl bg-primary/10 font-bold text-primary text-lg">
                {initials}
              </AvatarFallback>
  ...
  <>
    ...
    >
      <div className="absolute top-4 right-4 z-10">
        <button onClick={onClose} className="w-9 h-9 flex items-center justify-center rounded-xl hover:bg-accent">
          <X size={22} />
        </button>
      </div>
