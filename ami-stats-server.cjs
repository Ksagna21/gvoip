...
function amiSession(opts, actions) {
  return new Promise((resolve, reject) => {
    ...
    const makeDonePredicate = (action) => {
      ...
      // FreePBX / Asterisk récents : PJSIP (chan_sip retiré → SIPpeers inconnu)
      if (name === "pjsipshowendpoints") {
        return (obj) => obj.Event === "EndpointListComplete";
      }
    ...
    socket.on("close", () => {
      if (!done) { done = true; clearTimeout(timeout); resolve(results); }
    });
  });
}
...
/** Compte les peers SIP (chan_sip) depuis SIPpeers → PeerEntry */
...
/**
 * Compte les endpoints PJSIP depuis PJSIPShowEndpoints → EndpointList / EndpointListComplete
 * "online" ≈ au moins une ligne Contacts non vide (registration présente).
 */
function parsePJSIPEndpoints(blocks) {
  let online = 0, total = 0;
  for (const b of blocks) {
    const obj = parseBlock(b);
    if (obj["Event"] !== "EndpointList") continue;
    if (obj["ObjectType"] && obj["ObjectType"] !== "endpoint") continue;
    total++;
    const contacts = (obj["Contacts"] || "").trim();
    if (contacts && contacts !== "-" && !/^0\b/.test(contacts)) online++;
  }
  return { sip_peers_total: total, sip_peers_online: online };
}
...
app.post("/api/ipbx/stats", async (req, res) => {
  ...
  try {
    const actions = [
      ...
      // PJSIP (FreePBX moderne) ; SIPpeers n'existe plus si chan_sip est absent
      { Action: "PJSIPShowEndpoints", ActionID: "pjsip_endpoints" },
    ];
    ...
    const pjsipBlocks = (amiByAction.pjsip_endpoints?.blocks) || [];

    const coreInfo = parseCoreStatus(coreBlocks);
    const sipInfo = parsePJSIPEndpoints(pjsipBlocks);
